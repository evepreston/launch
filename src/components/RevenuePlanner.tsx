...
export default function RevenuePlanner() {
  ...
  const field = (
    ...
    <div>
      ...
      <div className="flex items-center gap-2">
        {opts.prefix && <span className="text-[#5a5a52]">{opts.prefix}</span>}
        <input
          type="number"
          value={value}
          min={opts.min ?? 0}
        ...
        />
        {opts.suffix && <span className="text-[#5a5a52] whitespace-nowrap">{opts.suffix}</span>}
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#f0f2e9] rounded-2xl p-6 mt-6">
      <h3 className="font-semibold text-[#2c2c2a] mb-1">Revenue Planner</h3>
      ...
      <div className="mt-6 grid sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-4 text-center border border-[#e6ddc9]">
          <p className="text-sm text-[#5a5a52]">Estimated gross annual income</p>
          <p className="text-2xl font-semibold text-[#7d9b76]">
            ${grossAnnual.toLocaleString(undefined, { maximumFractionDigits: 0 })}
          </p>
        </div>
        <div className="bg-white rounded-xl p-4 text-center border border-[#e6ddc9]">
          <p className="text-sm text-[#5a5a52]">Estimated monthly income</p>
          <p className="text-2xl font-semibold text-[#7d9b76]">
